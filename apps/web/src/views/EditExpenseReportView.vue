    ...
    <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
      ...
      <template v-else-if="loaded">
        ...
        <form @submit.prevent="handleSubmit" class="p-8 space-y-6">
          ...
          <div v-if="form.advance_amount !== ''">
            <label class="block text-sm font-medium text-gray-700 mb-2">Data do Adiantamento</label>
            <input
...
onMounted(async () => {
  ...
  form.value.advance_amount = r.advance_amount != null && r.advance_amount > 0 ? String(r.advance_amount / 100) : ''
  form.value.advance_date = r.advance_date ? r.advance_date.substring(0, 10) : ''
  loaded.value = true
})
...
async function handleSubmit() {
  ...
  const data: Record<string, any> = {
    ...
    advance_amount: form.value.advance_amount !== '' ? Math.round(parseFloat(form.value.advance_amount) * 100) : 0,
    advance_date: form.value.advance_date || null,
  }
